...
class NotificationServiceImplTest {
        ...
        @Test
        @DisplayName("Should send push notifications to all subscribed users except the creator")
        void processNotificationMessage_ShouldSendPushNotifications_WhenSubscribersExist() {
                List<UserDto> subscribers = List.of(subscriber1, creator);
                when(userService.getUsersByChannelAndCategory(eq(NotificationChannel.PUSH),
                                eq(NotificationCategory.MOVIES)))
                                .thenReturn(subscribers);
                notificationService.processNotificationMessage(sampleMessage);
                verify(userService).getUsersByChannelAndCategory(NotificationChannel.PUSH, NotificationCategory.MOVIES);
        }
